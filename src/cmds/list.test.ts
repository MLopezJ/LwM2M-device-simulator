...
describe('List', () => {
	let objectList = assetTrackerFirmwareV2
	beforeEach(() => {
		objectList = assetTrackerFirmwareV2
	})

	it('Should list all the object list when no URL is provided', async () => {
		const param = undefined
		expect(await list(param, objectList)).toMatchObject(objectList)
	})

	it('Should return undefined when input do not follow LwM2M object syntax', async () => {
		const param = 'test'
		const expectedResult = undefined
		expect(await list(param, objectList)).toBe(expectedResult)
	})

	describe('single instance', () => {
		it('Should list object', async () => {
			const param = '/3'
			const expectedResult = objectList[Device_3_urn] ?? ''
			expect(await list(param, objectList)).toMatchObject(expectedResult)
		})
	...
	describe('multiple instance', () => {
		it('Should list object', async () => {
			const param = '/3303'
			const expectedResult = objectList[Temperature_3303_urn] ?? ''
			expect(await list(param, objectList)).toMatchObject(expectedResult)
		})
